...
class WriterMapper extends Mapper {
    ...
    function getWritersByName ($name) {
        ...
        //print $selectStmt . " <- dit is de sql methode die we moeten aanpassen voor volgende methode";
        return $this->getCollection($writers);
    }
    
    function getWritersByNameAndDoB ($name, $date_of_birth) {
        $con = $this->getConnectionManager();
        $selectStmt = "SELECT a.*, b.* from person a, writer b where a.uri = b.writer_uri and a.full_name like " ."\"" . $name . "%" . "\" AND a.birth_date = "."\"".$date_of_birth."\"";        
        $writers = $con->executeSelectStatement($selectStmt, array()); 
        // print $selectStmt;
        return $this->getCollection($writers);
    }
    ...
    function getWritersByNameAndDoBAndCountry ($name, $date_of_birth, $country) {
        ...
        $selectStmt = "SELECT a.*, b.* from person a, writer b where a.uri = b.writer_uri and a.full_name like " ."\"" . $name . "%" . "\" AND a.birth_date = "."\"".$date_of_birth."\" AND  a.uri IN (SELECT d.person_uri from (has_citizenship d join country c on c.iso_code = d.country_iso_code) where c.iso_code = "."\"".$country."\")";
